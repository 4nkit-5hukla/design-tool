...
const Contrast = () => {
  ...
  useEffect(() => {
    selectedEl && setContrast(selectedEl.contrast / 10);
  }, [selectedEl]);
  ...
  return (
    <Box display="flex" flexDirection="column" rowGap={1} marginBottom={0.75}>
      <Box alignItems="center" display="flex" justifyContent="space-between">
        ...
        <TextField
          ...
          onChange={({ target: { value } }) => {
            setContrast(parseInt(value) ? parseInt(value) : 0);
            updateElement(
              {
                ...selectedEl,
                contrast: parseInt(value) ? parseInt(value) * 10 : 0,
              },
              { saveHistory: false }
            );
          }}
      ...
      <Slider
        ...
        value={[contrast < 0 ? contrast : 0, contrast > 0 ? contrast : 0]}
        step={0.5}
        min={-10}
        max={10}
        ...
        onChangeCommitted={() => {
          updateElement({ ...selectedEl, contrast: contrast * 10 });
        }}
        sx={{
          color: "#24272C4D",
