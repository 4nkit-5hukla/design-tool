    ...
    <Box display="flex" flexDirection="column" rowGap={1} marginBottom={0.75}>
      <Box alignItems="center" display="flex" justifyContent="space-between">
        ...
        <TextField
          ...
          onChange={({ target: { value } }) => {
            setBlue(parseInt(value) ? parseInt(value) : 0);
            updateElement(
              { ...selectedEl, blue: parseInt(value) ? parseInt(value) : 0 },
              { saveHistory: false }
            );
          }}
      ...
      <Slider
        ...
        onChange={(event: Event, newValue: number | number[]) => {
          ...
          updateElement({ ...selectedEl, blue: value }, { saveHistory: false });
          setBlue(value);
        }}
        onChangeCommitted={() => {
          updateElement({ ...selectedEl, blue });
        }}
        sx={{
          color: "#24272C4D",
