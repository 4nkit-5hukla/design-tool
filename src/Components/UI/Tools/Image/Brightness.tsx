...
const Brightness: FC = () => {
  ...
  useEffect(() => {
    selectedEl && setBrightness(selectedEl.brightness * 10);
  }, [selectedEl]);
  ...
  return (
    <Box display="flex" flexDirection="column" rowGap={1} marginBottom={0.75}>
      <Box alignItems="center" display="flex" justifyContent="space-between">
        ...
        <TextField
          ...
          onChange={({ target: { value } }) => {
            setBrightness(parseInt(value) ? parseInt(value) : 0);
            updateElement(
              {
                ...selectedEl,
                brightness: parseInt(value) ? parseInt(value) / 10 : 0,
              },
              { saveHistory: false }
            );
          }}
      ...
      <Slider
        ...
        onChangeCommitted={() => {
          updateElement({ ...selectedEl, brightness: brightness / 10 });
        }}
        sx={{
          color: "#24272C4D",
