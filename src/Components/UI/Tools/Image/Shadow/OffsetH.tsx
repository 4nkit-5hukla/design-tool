...
export const OffsetH: FC<ShadowProps> = ({ shadowEnabled, selectedEl, updateElement }) => {
  const { toggleEditingData } = useAppState();
  const [offsetX, setOffsetX] = useState<number>(0);

  useEffect(() => {
    if (selectedEl) {
      setOffsetX(selectedEl.shadowOffsetX);
    }
  }, [selectedEl]);
  ...
  return (
    <Box display="flex" flexDirection="column" rowGap={1} marginBottom={0.75}>
      <Box alignItems="center" display="flex" justifyContent="space-between">
        ...
        <TextField
          ...
          onChange={({ target: { value } }) => {
            setOffsetX(parseInt(value) ? parseInt(value) : 0);
            updateElement(
              {
                ...selectedEl,
                shadowOffsetX: parseInt(value) ? parseInt(value) : 0,
              },
              { saveHistory: false }
            );
          }}
      ...
      <Slider
        getAriaLabel={() => "Offset Horizontal"}
        value={[offsetX < 0 ? offsetX : 0, offsetX > 0 ? offsetX : 0]}
        disabled={!shadowEnabled}
        step={1}
        min={-10}
        max={10}
        className={offsetX < 0 ? "value-negative" : "value-positive"}
        onChange={(event: Event, newValue: number | number[]) => {
          ...
          const value =
            values[0] !== 0
              ...
              : values[1];
          updateElement(
            { ...selectedEl, shadowOffsetX: value },
            { saveHistory: false }
          );
          setOffsetX(value);
        }}
        onChangeCommitted={() => {
          updateElement({ ...selectedEl, shadowOffsetX: offsetX });
        }}
        sx={{
          color: "#24272C4D",
