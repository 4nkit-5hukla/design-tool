...
export const OffsetV: FC<ShadowProps> = ({ shadowEnabled, selectedEl, updateElement }) => {
  ...
  useEffect(() => {
    if (selectedEl) {
      setOffsetY(selectedEl.shadowOffsetY);
    }
  }, [selectedEl]);
  ...
  return (
    <Box display="flex" flexDirection="column" rowGap={1} marginBottom={0.75}>
      <Box alignItems="center" display="flex" justifyContent="space-between">
        ...
        <TextField
          ...
          onChange={({ target: { value } }) => {
            setOffsetY(parseInt(value) ? parseInt(value) : 0);
            updateElement(
              {
                ...selectedEl,
                shadowOffsetY: parseInt(value) ? parseInt(value) : 0,
              },
              { saveHistory: false }
            );
          }}
      ...
      <Slider
        getAriaLabel={() => "Offset Vertical"}
        value={[offsetY < 0 ? offsetY : 0, offsetY > 0 ? offsetY : 0]}
        ...
        onChangeCommitted={() => {
          updateElement({ ...selectedEl, shadowOffsetY: offsetY });
        }}
        sx={{
          color: "#24272C4D",
