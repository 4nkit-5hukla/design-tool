...
const SVGColor = () => {
  ...
  const { currentColor, setEditTool, setCurrentColor, toggleEditSelected } =
    useAppState();
  const [elementColors, setElementColors] = useState<string[]>([]);
  ...
  const switchColor = (color: string) => {
    const newSelectedElementData = JSON.parse(
      JSON.stringify(selectedEl.data).replaceAll(currentColor, color)
    );
    const newElementColors = elementColors
      .map((elColor: string) => (elColor === currentColor ? color : elColor))
      .filter(
        (elColor, index, allColors) => allColors.indexOf(elColor) === index
      );
    setCurrentColor(color);
    updateElement({
      id: selectedEl.id,
      colors: newElementColors,
      data: newSelectedElementData,
    });
  };

  useEffect(() => {
    if (selectedEl) {
      if (selectedEl.colors) {
        setElementColors(selectedEl.colors);
      }
    }
  }, [selectedEl]);
      ...
      >
        <ChromePicker
          ...
          onChange={({ hex }: any) => {
            ...
            const newSelectedElementData: any = JSON.parse(
              JSON.stringify(selectedEl.data).replaceAll(currentColor, newColor)
            );
            const newElementColors: string[] = elementColors
              .map((elColor: string) =>
                elColor === currentColor ? newColor : elColor
              )
              .filter(
                (elColor, index, allColors) =>
                  allColors.indexOf(elColor) === index
              );
            updateElement(
              {
                id: selectedEl.id,
