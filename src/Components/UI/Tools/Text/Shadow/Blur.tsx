...
export const Blur = ({ shadowEnabled, selectedEl, updateElement }: any) => {
  const { toggleEditingData } = useAppState();
  const [blur, setBlur] = useState<number>(0);

  useEffect(() => {
    selectedEl && setBlur(selectedEl.shadowBlur);
  }, [selectedEl]);
    ...
    <Box display="flex" flexDirection="column" rowGap={1} marginBottom={0.75}>
      <Box alignItems="center" display="flex" justifyContent="space-between">
        ...
        <TextField
          ...
          onChange={({ target: { value } }) => {
            setBlur(parseInt(value) ? parseInt(value) : 0);
            updateElement(
              {
                ...selectedEl,
                shadowBlur: parseInt(value) ? parseInt(value) : 0,
              },
              { saveHistory: false }
            );
          }}
      ...
      <Slider
        ...
        onChange={(event: Event, newValue: any) => {
          ...
          updateElement(
            { ...selectedEl, shadowBlur: value },
            { saveHistory: false }
          );
          setBlur(value);
        }}
        onChangeCommitted={() => {
          updateElement({ ...selectedEl, shadowBlur: blur });
        }}
        sx={{
          color: "#24272C4D",
