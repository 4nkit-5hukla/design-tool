      ...
      ? ((
          {
            ...
            ...groupProps
          }: { descendants: unknown[]; globalCompositeOperation?: string; [key: string]: unknown },
          x: number = 0,
          y: number = 0
        ) => (
          <Group key={index} globalCompositeOperation={globalCompositeOperation} {...groupProps} {...{ x, y }}>
            {descendants.map(mapCallBack)}
          </Group>
        ))(rest)
      : (({ d, ...pathProps }: { d: string; [key: string]: unknown }, x: number = 0, y: number = 0) => (
          <Path key={index} data={svgpath(d).scale(2).toString()} {...pathProps} {...{ x, y }} />
        ))(rest);
      ...
      {isSelected && (
        <Portal selector=".top-layer" enabled={isSelected}>
          <Transformer
            ...
            enabledAnchors={!lock ? ["top-left", "top-right", "bottom-right", "bottom-left"] : []}
          />
        </Portal>
      )}
    </Fragment>
