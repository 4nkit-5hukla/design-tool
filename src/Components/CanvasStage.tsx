...
const CanvasStage = () => {
  const { canvas, containerRef, setCurrentColor, toggleEditSelected, setEditTool, editText, toggleEditText, stageRef } =
    useAppState();
  ...
  useEffect(() => {
    if (containerRef.current && wrapperRef.current) {
      ...
      containerEl.scrollTop = Math.abs(stageEl.clientHeight - containerEl.clientHeight) / 2;
      containerEl.scrollLeft = Math.abs(stageEl.clientWidth - containerEl.clientWidth) / 2;
    }
  }, [zoom]);
  ...
  useEffect(() => {
    ...
    const minContainerSide = Math.min(containerEl.clientHeight, containerEl.clientWidth);
